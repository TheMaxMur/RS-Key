...
The attestation is packed **basic** attestation, ES256 by the device key, so it
carries no ML-DSA signature to cross-verify; the assertion signature carries the
cross-verify, including the tamper negative control.
"""
import os
import sys
import time
...
def main():
    ...
    try:
        ...
        for label, alg, dil, pyca_cls, pk_len, sig_len in SETS:
            ...
            # Packed basic attestation is ES256 by the device key, so there is no
            # ML-DSA signature in the statement to cross-verify — only the assertion.
            assert fmt == "packed", f"{label} fmt={fmt!r}"
            assert att["alg"] == -7 and att["x5c"], f"{label} attStmt {att}"
            ...
            # Negative control: a one-bit flip in the assertion signature must be
            # rejected by BOTH verifiers.
            good_ad, good_sig = ga_ad, ga_sig
