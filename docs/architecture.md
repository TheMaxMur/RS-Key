...
**The image is one edge out of date.** `piv→openpgp` is gone: the EC key type PIV reached for — `PrivKey` over `Curve`, the `[curve_id] ‖ scalar` blob both applets seal — now sits in `rsk-ec` beside `rsk-rsa`, so no applet names a sibling in code or in a manifest. Six `rsk_<applet>::` references survive across the applet tier, every one of them inside a comment where one applet explains its ordering by pointing at a sibling. Nothing mechanical holds the rule yet — the `deny.toml` stanza that will, and this SVG, are the last step of the crate-layout work, not this one.
