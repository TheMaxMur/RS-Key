...
fn a_wrong_pin_kills_an_outstanding_token_through_every_door() {
    ...
    for (label, wrong) in [
        ...
        let token = mint_cm_token(&mut fs, &mut rng, &mut state, &plat, b"1234");
        assert!(
        ...
        let req = match wrong {
            0 => plat2.get_token_perms_req(b"9999", PERM_CM as u64),
            1 => plat2.get_token_req(b"9999"),
            _ => plat2.change_pin_req(b"9999", b"5678"),
        };
        let mut out = [0u8; 256];
        assert_eq!(
...
fn the_persistent_token_is_not_swept_along() {
    ...
    assert_eq!(
        run(
            ...
            &mut rng,
            &mut state,
            &plat.get_token_perms_req(b"9999", PERM_CM as u64),
            &mut out,
        ),
        Err(CtapError::PinInvalid),
    );
...
fn the_surrounding_cells_are_unchanged() {
    ...
    let plat2 = key_agreement(&mut fs, &mut rng, &mut state, PinProto::Two, 2);
    let token = mint_cm_token(&mut fs, &mut rng, &mut state, &plat2, b"1234");
